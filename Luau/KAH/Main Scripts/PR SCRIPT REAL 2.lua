local Settings = {
    ...
    ["Default Whitelisted"] = {"atprog","IceStuds","IronWolf_Az"}, -- People whitelisted by default
    ["Default Banned"] = {"SlenderMan990921","jhjssikeksms"}, -- People banned by default [Buggy]
    ["Default Softlocked"] = {}, -- People softlocked by Default
    ["Default GearBanned"] = {"ThomasPlayesGames"}, -- People GearBanned by Default
    ["Player Crash Settings"] = {["Sil"] = false, ["Players"] = {"jhjssikeksms"}}, -- Automatically crashes server if one of these players are in it
	...
	["jhjssikeksms"] = "characteradded jhjssikeksms runcommand .rkick jhjssikeksms"
    }
}
...
addCommand("rkick",{"player"},function(args)
	...
	for i,v in pairs(game.Players.LocalPlayer.Character.Humanoid:GetPlayingAnimationTracks()) do v:Stop()end
            ...
            game.Players:Chat(":setgrav ".. v.Name .." 3500")
            game.Players:Chat(":thaw ".. v.Name)
            task.wait(.23)
            game.Players:Chat(":size " .. v.Name .. " 2.13527345204524123423734") -- this is so annoying with double chat.
            task.wait(.35)
...
addCommand("rkicksound",{"player","soundid"},function(args)
	local cacheAntiAbuse = Loops.antiabuse
...
addCommand("kicksound",{"player"},function(args)
    runCommand(prefix.."rkicksound",args)
end)
...
game.Players.LocalPlayer.Chatted:Connect(function(age)
if string.sub(age, 1, 5) == ".age " then
local name = string.sub(age, 6)
game.Players:Chat("iy")
game.Players:Chat("iyc chatage " ..name)
end
end)
...
plr.Chatted:Connect(function(rkickv3)
 if string.sub(rkickv3, 1, 8) == ".srkick " then
 local name = string.sub(rkickv3, 9)
local srkick = true
while true do
 for i = 1,128 do
 game.Players:Chat("rocket/"..name.."/"..name.."/"..name)
 end
 end
 end
 end)

plr.Chatted:Connect(function(rkickv2)
 if string.sub(rkickv2, 1, 8) == ".rkick2 " then
 local name = string.sub(rkickv2, 9)
...
addCommand("unreanimate",{},function()
	...
	workspace["_charPRStupidAdmin"]:Destroy()
	game.Players:Chat("reset me")
end)
...
addCommand("trap",{"player","time"},function(args)
    ...
    elseif game.Players.LocalPlayer.Character:FindFirstChild("PortableJustice") then
        ...
        game.Players:Chat("freeze " ..PlayerName)
