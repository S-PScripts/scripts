...
# Process each line: filter by prefix, remove everything before '==', and remove 'then' and comments
modified_lines = []
for line in lines:
    if prefix in line:
        # Find the index of '=='
        ...
        if index != -1:
            ...
            # Remove the word 'then' if it exists
            modified_line = modified_line.split('then')[0].strip()  # Keep everything before 'then'
            # Remove comments (if any) starting from '--'
            modified_line = modified_line.split('--')[0].strip()  # Keep everything before '--'
